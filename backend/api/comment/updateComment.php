...
if ($current_username !== $owner_username && !$is_admin) {
    sendResponse('error', '无权修改该评论');
}

// 执行更新
$content_safe = mysqli_real_escape_string($conn, $content);
$update_query = "UPDATE comments SET content = '$content_safe' WHERE id = '$comment_id_safe'";

if (mysqli_query($conn, $update_query)) {
    sendResponse('success', '评论修改成功');
} else {
    sendResponse('error', '修改失败: ' . mysqli_error($conn));
}
